...
<script>
    function select_student1() {
        ...
        var t = $('#student_table').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url()?>members/list_students/"+classID+"/"+sectionID+"/"+batchID,
                "type": 'GET',
            }
        });
    }
...
<script>
    $(document).ready(function () {
        ...
        var t = $('#student_table').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url()?>members/list_students/",
                "type": 'GET',
            }
        });
    });
...
<script>
    ...
    {
        var studentID = sel;
        var is_active = document.getElementById(sel).value;
        if (studentID != "" && is_active != "")
        {
            $.ajax({
                url: "<?= base_url("members")?>",
                method: "POST",
                data:"studentID="+studentID+"&is_active="+is_active,
                success:function(res){
