    ...
    <div class="col-sm-8">
        <div class="white-box">
            ...
            <form class="form-horizontal" method="post" >
                ...
                <div class="form-group">
                    <label class="col-sm-4">Batch (optional) : </label>
                    <div class="col-sm-8">
                        <select class="form-control" style="font-weight: 500" name="batchID" id="batchID">
                            <option value="">--select--</option>
                            <?php
                            foreach ($batch as $c) { ?>
                                <option <?php if ($student_info->batchID == $c->batchID){echo "selected" ;} ?> value="<?= $c->batchID; ?>"><?= $c->batch; ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
...
<script>
    function validate_username(e) {
        ...
        if (new_username == ""){
            ...
            $.ajax({
                method : "post",
                url : "<?= base_url() .'members/check_username/' ?>",
    ...
    function username_change(e) {
        ...
        if (new_username == ""){
            ...
            $.ajax({
                method : "post",
                url : "<?= base_url() .'members/change_username/'.$student_info->studentID ?>",
                data:"username="+new_username,
                success:function (res) {
                    if (res == "failure"){
