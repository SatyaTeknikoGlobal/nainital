...
<script>
    function select_student1() {
        ...
        var t = $('#student_table').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url()?>accounts/list_students/"+classID+"/"+sectionID+"/"+batchID,
                "type": 'GET',
            }
        });
    }
...
<script>
    $(document).ready(function () {
        ...
        var t = $('#student_table').DataTable( {
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?=base_url()?>accounts/list_students/",
                "type": 'GET',
            }
        });
    });
...
<script>
    ...
    {
        var studentID = sel;
        var concessionID = document.getElementById(sel).value;
        if (studentID != "" && concessionID != "")
        {
            $.ajax({
                url: "<?= base_url("accounts/allocate_concession")?>",
                method: "POST",
                data:"studentID="+studentID+"&concessionID="+concessionID,
                success:function(res){
